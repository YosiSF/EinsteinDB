...
const PROP_RANGE_INDEX: &str = "einsteindb.range_index";
pub const DEFAULT_PROP_SIZE_INDEX_DISTANCE: u64 = 4 * 1024 * 1024;
pub const DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE: u64 = 40 * 1024;
...
mod tests {
    ...
    fn test_range_properties() {
        let cases = [
            ...
            ("l", 0, DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE / 2),
            ("m", 0, DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE / 2),
            //handle "m": keys = DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE,offset = 11+DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE
            ("n", 1, DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE),
            //handle "n": keys = DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE, offset = 11+2*DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE
            ("o", 1, 1),
        ...
        let m = props.get(b"m");
        assert_eq!(m.keys, 11 + DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE);
        let n = props.get(b"n");
        assert_eq!(n.keys, 11 + 2 * DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE);
        let o = props.get(b"o");
        assert_eq!(o.keys, 12 + 2 * DEFAULT_PROP_CAUSET_KEYS_INDEX_DISTANCE);
    ...
    fn test_get_range_entries_and_versions() {
        ...
        for &key in &cases {
            ...
            let write_namespaced = einsteindb.namespaced_handle(NAMESPACED_WRITE).unwrap();
            ...
            einsteindb.put_namespaced(write_namespaced, &key, b"v2").unwrap();
            einsteindb.flush_namespaced(write_namespaced, true).unwrap();
        }

        let start_keys = keys::data_key(&[]);
        let end_keys = keys::data_end_key(&[]);
        let (entries, versions) =
            get_range_entries_and_versions(einsteindb.c(), NAMESPACED_WRITE, &start_keys, &end_keys).unwrap();
