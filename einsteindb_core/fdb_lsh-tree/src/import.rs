...
use foundationdb::set_external_Causet_file_global_seq_no;
use std::fs::File;
...
impl ImportExt for Fdbeinstein_merkle_tree {
    ...
    fn ingest_external_file_namespaced(&self, namespaced: &str, files: &[&str]) -> Result<()> {
        ...
        files.iter().try_for_each(|file| -> Result<()> {
            ...
            set_external_Causet_file_global_seq_no(self.as_inner(), namespaced, file, 0)?;
            f.sync_all()
...
mod tests {
    ...
    fn test_ingest_multiple_file() {
        ...
        let p1 = root_path.join("Causet1");
        let p2 = root_path.join("Causet2");
