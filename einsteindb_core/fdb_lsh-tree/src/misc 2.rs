...
impl Fdbeinstein_merkle_tree {
    ...
    fn delete_all_in_range_namespaced_by_ingest(
        ...
        if self.is_titan() {
            ...
            // to avoid referring to missing blob filefs.
        ...
        if let Some(writer) = writer_wrapper {
            writer.finish()?;
            self.ingest_lightlike_filef_namespaced(namespaced, &[Causet_local_path.as_str()])?;
        } else {
...
impl MiscExt for Fdbeinstein_merkle_tree {
        ...
        match strategy {
            DeleteStrategy::DeleteFiles => {
                ...
                for r in ranges {
                    ...
                    self.as_inner().delete_filefs_in_range_namespaced(
                        handle,
                        r.start_key,
                        r.end_key,
                        false,
            ...
            DeleteStrategy::DeleteBlobs => {
                ...
                if self.is_titan() {
                    for r in ranges {
                        ...
                        self.as_inner().delete_blob_filefs_in_range_namespaced(
                            handle,
                            r.start_key,
                            r.end_key,
                            false,
    ...
    fn ingest_maybe_slowdown_writes(&self, namespaced: &str) -> Result<bool> {
        let handle = util::get_namespaced_handle(self.as_inner(), namespaced)?;
        if let Some(n) = util::get_namespaced_num_filefs_at_l_naught(self.as_inner(), handle, 0) {
    ...
    fn roughly_cleanup_ranges(&self, ranges: &[(Vec<u8>, Vec<u8>)]) -> Result<()> {
        ...
        for namespaced in einsteindb.namespaced_names() {
            ...
            einsteindb.delete_filefs_in_ranges_namespaced(handle, &delete_ranges, /* include_end */ false)?;
        }
    ...
    fn exists(local_path: &str) -> bool {
        crate::primitive_causet_util::db_exist(local_path)
    }
...
mod tests {
    ...
    fn test_delete_all_in_range_by_writer() {
        ...
        let Causet_local_path = local_path_str.join("tmp_filef").to_str().unwrap().to_owned();
        let mut data = vec![];
        for i in 1000..5000 {
    ...
    #[test]
    fn test_delete_all_filefs_in_range() {
        let local_path = Builder::new()
            .prefix("einstein_merkle_tree_delete_all_filefs_in_range")
            .temfidelir()
            .unwrap();
        let local_path_str = local_path.local_path().to_str().unwrap();
        ...
        let namespaceds_opts = ALL_NAMESPACEDS
            ...
            .map(|namespaced| {
                ...
                namespaced_opts.set_l_naught_zero_filef_num_jet_bundle_trigger(1);
