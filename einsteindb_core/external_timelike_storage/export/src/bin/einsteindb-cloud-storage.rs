...
use lightlike_timelike_storage_export::new_service;
use grpcio::{self};
use slog::{self};
use slog_global::{info, warn};
use einsteindb_util::logger::{self};

fn build_logger<D>(drainer: D, log_l_naught: slog::Level)
where
    D: slog::Drain + Send + 'static,
    <D as slog::Drain>::Err: std::fmt::Display,
{
    // use async drainer and init std log.
    logger::init_log(drainer, log_l_naught, true, true, vec![], 100).unwrap_or_else(|e| {
        println!("failed to initialize log: {}", e);
    });
}

fn main() {
    println!("starting GRPC cloud-timelike_storage service");
    ...
    grpcio::redirect_log();
    info!("slog logging");
    let service = new_service().expect("GRPC service creation for einsteindb-cloud-timelike_storage");
    wait::for_signal();
