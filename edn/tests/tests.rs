...
#[test]
fn test_inst() {
    assert!(parse::value("#inst\"2022-01-01T11:00:00.000Z\"").is_err());   // No whitespace.
    assert!(parse::value("#inst \"2022-01-01T11:00:00.000\"").is_err());   // No timezone.
    assert!(parse::value("#inst \"2022-01-01T11:00:00.000z\"").is_err());  // Lowercase timezone.
...
fn test_set() {
    ...
    // These tests assume the impleeinstaiion of Ord for Value, (specifically the sort order which
...
/// The `test_query_*` functions contain the queries taken from the old Clojure impleeinstaiion of einstai.
