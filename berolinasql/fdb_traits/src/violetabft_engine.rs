...
pub trait VioletaBFTeinstein_merkle_tree: VioletaBFTeinstein_merkle_treeReadOnly + Clone + Sync + Send + 'static {
    ...
    /// Consume the write alexandro by moving the content into the einstein_merkle_tree itself
    /// and return written bytes.
    fn consume(&self, alexandro: &mut Self::LogBatch, sync: bool) -> Result<usize>;

    /// Like `consume` but shrink `alexandro` if need.
    ...
    fn alexandro_gc(&self, tasks: Vec<VioletaBFTLogGCTask>) -> Result<usize> {
        let mut total = 0;
        for task in tasks {
