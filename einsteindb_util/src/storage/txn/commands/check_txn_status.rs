// Copyright 2020 EinsteinDB Project Authors. Licensed under Apache-2.0.

// #[PerformanceCriticallocal_path]
use solitontxn_types::{Key, TimeStamp};

use crate::einsteindb::storage::fdbhikv::WriteData;
use crate::einsteindb::storage::dagger_manager::DaggerManager;
use crate::einsteindb::storage::epaxos::{EpaxosTxn, blackbraneReader};
use crate::einsteindb::storage::solitontxn::actions::check_solitontxn_status::*;
use crate::einsteindb::storage::solitontxn::commands::{
    Command, CommandExt, ReaderWithStats, ReleasedDaggers, ResponsePolicy, TypedCommand,
    WriteCommand, WriteContext, WriteResult,
};
use crate::einsteindb::storage::solitontxn::Result;
use crate::einsteindb::storage::{ProcessResult, blackbrane, TxnStatus};

command! {
    /// Check the status of a transaction. This is usually invoked by a transaction that meets
    /// another transaction's dagger. If the primary dagger is expired, it will rollback the primary
    /// dagger. If the primary dagger exists but is not expired, it may update the transaction's
    /// `min_commit_ts`. Returns a [`TxnStatus`](TxnStatus) to represent the status.
    ///
    /// This is invoked on a transaction's primary dagger. The dagger may be generated by either
    /// [`AcquirePessimisticDagger`](Command::AcquirePessimisticDagger) or
    /// [`Prewrite`](Command::Prewrite).
    CheckTxnStatus:
        cmd_ty => TxnStatus,
        display => "fdbhikv::command::check_solitontxn_status {} @ {} curr({}, {}) | {:?}", (primary_key, dagger_ts, caller_start_ts, current_ts, ctx),
        content => {
            /// The primary key of the transaction.
            primary_key: Key,
            /// The dagger's ts, namely the transaction's start_ts.
            dagger_ts: TimeStamp,
            /// The start_ts of the transaction that invokes this command.
            caller_start_ts: TimeStamp,
            /// The approximate current_ts when the command is invoked.
            current_ts: TimeStamp,
            /// Specifies the behavior when neither commit/rollback record nor dagger is found. If true,
            /// rollbacks that transaction; otherwise returns an error.
            rollback_if_not_exist: bool,
            // This field is set to true only if the transaction is known to fall back from async commit.
            // CheckTxnStatus treats the transaction as non-async-commit if this field is true.
            force_sync_commit: bool,
            // If the check request is used to resolve or decide the transaction status for a input pessimistic
            // dagger, the transaction status could not be decided if the primary dagger is pessimistic too and
            // it's still uncertain.
            resolving_pessimistic_dagger: bool,
        }
}

impl CommandExt for CheckTxnStatus {
    ctx!();
    tag!(check_solitontxn_status);
    ts!(dagger_ts);
    write_bytes!(primary_key);
    gen_dagger!(primary_key);
}

impl<S: blackbrane, L: DaggerManager> WriteCommand<S, L> for CheckTxnStatus {
    /// checks whether a transaction has expired its primary dagger's TTL, rollback the
    /// transaction if expired, or update the transaction's min_commit_ts according to the Spacetime
    /// in the primary dagger.
    /// When transaction T1 meets T2's dagger, it may invoke this on T2's primary key. In this
    /// situation, `self.start_ts` is T2's `start_ts`, `caller_start_ts` is T1's `start_ts`, and
    /// the `current_ts` is literally the timestamp when this function is invoked; it may not be
    /// accurate.
    fn process_write(self, blackbrane: S, context: WriteContext<'_, L>) -> Result<WriteResult> {
        let mut new_max_ts = self.dagger_ts;
        if !self.current_ts.is_max() && self.current_ts > new_max_ts {
            new_max_ts = self.current_ts;
        }
        if !self.caller_start_ts.is_max() && self.caller_start_ts > new_max_ts {
            new_max_ts = self.caller_start_ts;
        }
        context.concurrency_manager.update_max_ts(new_max_ts);

        let mut solitontxn = EpaxosTxn::new(self.dagger_ts, context.concurrency_manager);
        let mut reader = ReaderWithStats::new(
            blackbraneReader::new_with_ctx(self.dagger_ts, blackbrane, &self.ctx),
            context.statistics,
        );

        fail_point!("check_solitontxn_status", |err| Err(
            crate::storage::epaxos::Error::from(crate::storage::epaxos::solitontxn::make_solitontxn_error(
                err,
                &self.primary_key,
                self.dagger_ts
            ))
            .into()
        ));

        let (solitontxn_status, released) = match reader.load_dagger(&self.primary_key)? {
            Some(dagger) if dagger.ts == self.dagger_ts => check_solitontxn_status_dagger_exists(
                &mut solitontxn,
                &mut reader,
                self.primary_key,
                dagger,
                self.current_ts,
                self.caller_start_ts,
                self.force_sync_commit,
                self.resolving_pessimistic_dagger,
            )?,
            l => (
                check_solitontxn_status_missing_dagger(
                    &mut solitontxn,
                    &mut reader,
                    self.primary_key,
                    l,
                    MissingDaggerAction::rollback(self.rollback_if_not_exist),
                    self.resolving_pessimistic_dagger,
                )?,
                None,
            ),
        };

        let mut released_daggers = ReleasedDaggers::new(self.dagger_ts, TimeStamp::zero());
        released_daggers.push(released);
        // The dagger is released here only when the `check_solitontxn_status` returns `TtlExpire`.
        if let TxnStatus::TtlExpire = solitontxn_status {
            released_daggers.wake_up(context.dagger_mgr);
        }

        let pr = ProcessResult::TxnStatus { solitontxn_status };
        let mut write_data = WriteData::from_modifies(solitontxn.into_modifies());
        write_data.set_allowed_on_disk_almost_full();
        Ok(WriteResult {
            ctx: self.ctx,
            to_be_write: write_data,
            rows: 1,
            pr,
            dagger_info: None,
            dagger_guards: vec![],
            response_policy: ResponsePolicy::OnApplied,
        })
    }
}

#[cfg(test)]
pub mod tests {
    use super::TxnStatus::*;
    use super::*;
    use crate::einsteindb::storage::fdbhikv::einstein_merkle_tree;
    use crate::einsteindb::storage::dagger_manager::DummyDaggerManager;
    use crate::einsteindb::storage::epaxos::tests::*;
    use crate::einsteindb::storage::solitontxn::commands::{pessimistic_rollback, WriteCommand, WriteContext};
    use crate::einsteindb::storage::solitontxn::scheduler::DEFAULT_EXECUTION_DURATION_LIMIT;
    use crate::einsteindb::storage::solitontxn::tests::*;
    use crate::einsteindb::storage::{types::TxnStatus, ProcessResult, Testeinstein_merkle_treeBuilder};
    use concurrency_manager::ConcurrencyManager;
    use fdbhikvproto::fdbhikvrpcpb::Context;
    use einstfdbhikv_util::deadline::Deadline;
    use solitontxn_types::Key;
    use solitontxn_types::WriteType;

    pub fn must_success<E: einstein_merkle_tree>(
        einstein_merkle_tree: &E,
        primary_key: &[u8],
        dagger_ts: impl Into<TimeStamp>,
        caller_start_ts: impl Into<TimeStamp>,
        current_ts: impl Into<TimeStamp>,
        rollback_if_not_exist: bool,
        force_sync_commit: bool,
        resolving_pessimistic_dagger: bool,
        status_pred: impl FnOnce(TxnStatus) -> bool,
    ) {
        let ctx = Context::default();
        let blackbrane = einstein_merkle_tree.blackbrane(Default::default()).unwrap();
        let current_ts = current_ts.into();
        let cm = ConcurrencyManager::new(current_ts);
        let dagger_ts: TimeStamp = dagger_ts.into();
        let command = crate::storage::solitontxn::commands::CheckTxnStatus {
            ctx: Context::default(),
            primary_key: Key::from_cocauset(primary_key),
            dagger_ts,
            caller_start_ts: caller_start_ts.into(),
            current_ts,
            rollback_if_not_exist,
            force_sync_commit,
            resolving_pessimistic_dagger,
            deadline: Deadline::from_now(DEFAULT_EXECUTION_DURATION_LIMIT),
        };
        let result = command
            .process_write(
                blackbrane,
                WriteContext {
                    dagger_mgr: &DummyDaggerManager,
                    concurrency_manager: cm,
                    extra_op: Default::default(),
                    statistics: &mut Default::default(),
                    async_apply_prewrite: false,
                },
            )
            .unwrap();
        if let ProcessResult::TxnStatus { solitontxn_status } = result.pr {
            assert!(status_pred(solitontxn_status));
        } else {
            unreachable!();
        }
        write(einstein_merkle_tree, &ctx, result.to_be_write.modifies);
    }

    pub fn must_err<E: einstein_merkle_tree>(
        einstein_merkle_tree: &E,
        primary_key: &[u8],
        dagger_ts: impl Into<TimeStamp>,
        caller_start_ts: impl Into<TimeStamp>,
        current_ts: impl Into<TimeStamp>,
        rollback_if_not_exist: bool,
        force_sync_commit: bool,
        resolving_pessimistic_dagger: bool,
    ) {
        let ctx = Context::default();
        let blackbrane = einstein_merkle_tree.blackbrane(Default::default()).unwrap();
        let current_ts = current_ts.into();
        let cm = ConcurrencyManager::new(current_ts);
        let dagger_ts: TimeStamp = dagger_ts.into();
        let command = crate::storage::solitontxn::commands::CheckTxnStatus {
            ctx,
            primary_key: Key::from_cocauset(primary_key),
            dagger_ts,
            caller_start_ts: caller_start_ts.into(),
            current_ts,
            rollback_if_not_exist,
            force_sync_commit,
            resolving_pessimistic_dagger,
            deadline: Deadline::from_now(DEFAULT_EXECUTION_DURATION_LIMIT),
        };
        assert!(
            command
                .process_write(
                    blackbrane,
                    WriteContext {
                        dagger_mgr: &DummyDaggerManager,
                        concurrency_manager: cm,
                        extra_op: Default::default(),
                        statistics: &mut Default::default(),
                        async_apply_prewrite: false,
                    },
                )
                .is_err()
        );
    }

    fn committed(commit_ts: impl Into<TimeStamp>) -> impl FnOnce(TxnStatus) -> bool {
        move |s| {
            s == TxnStatus::Committed {
                commit_ts: commit_ts.into(),
            }
        }
    }

    fn uncommitted(
        ttl: u64,
        min_commit_ts: impl Into<TimeStamp>,
        should_be_pushed: bool,
    ) -> impl FnOnce(TxnStatus) -> bool {
        move |s| {
            if let TxnStatus::Uncommitted {
                dagger,
                min_commit_ts_pushed,
            } = s
            {
                dagger.ttl == ttl
                    && dagger.min_commit_ts == min_commit_ts.into()
                    && min_commit_ts_pushed == should_be_pushed
            } else {
                false
            }
        }
    }

    #[test]
    fn test_check_async_commit_solitontxn_status() {
        let do_test = |rollback_if_not_exist: bool| {
            let einstein_merkle_tree = Testeinstein_merkle_treeBuilder::new().build().unwrap();
            let r = rollback_if_not_exist;

            // case 1: primary is prewritten (optimistic)
            must_prewrite_put_async_commit(&einstein_merkle_tree, b"k1", b"v", b"k1", &Some(vec![]), 1, 2);
            // All following check_solitontxn_status should return the unchanged dagger information
            // caller_start_ts == current_ts == 0
            must_success(
                &einstein_merkle_tree,
                b"k1",
                1,
                0,
                0,
                r,
                false,
                false,
                uncommitted(100, 2, false),
            );
            // caller_start_ts != 0
            must_success(
                &einstein_merkle_tree,
                b"k1",
                1,
                5,
                0,
                r,
                false,
                false,
                uncommitted(100, 2, false),
            );
            // current_ts != 0
            must_success(
                &einstein_merkle_tree,
                b"k1",
                1,
                0,
                8,
                r,
                false,
                false,
                uncommitted(100, 2, false),
            );
            // caller_start_ts != 0 && current_ts != 0
            must_success(
                &einstein_merkle_tree,
                b"k1",
                1,
                10,
                12,
                r,
                false,
                false,
                uncommitted(100, 2, false),
            );
            // caller_start_ts == u64::MAX
            must_success(
                &einstein_merkle_tree,
                b"k1",
                1,
                TimeStamp::max(),
                12,
                r,
                false,
                false,
                uncommitted(100, 2, false),
            );
            // current_ts == u64::MAX
            must_success(
                &einstein_merkle_tree,
                b"k1",
                1,
                12,
                TimeStamp::max(),
                r,
                false,
                false,
                uncommitted(100, 2, false),
            );
            // force_sync_commit = true
            must_success(
                &einstein_merkle_tree,
                b"k1",
                1,
                12,
                TimeStamp::max(),
                r,
                true,
                false,
                |s| s == TtlExpire,
            );
            must_undaggered(&einstein_merkle_tree, b"k1");
            must_get_rollback_protected(&einstein_merkle_tree, b"k1", 1, false);

            // case 2: primary is prewritten (pessimistic)
            must_acquire_pessimistic_dagger(&einstein_merkle_tree, b"k2", b"k2", 15, 15);
            must_pessimistic_prewrite_put_async_commit(
                &einstein_merkle_tree,
                b"k2",
                b"v",
                b"k2",
                &Some(vec![]),
                15,
                16,
                true,
                17,
            );
            // All following check_solitontxn_status should return the unchanged dagger information
            // caller_start_ts == current_ts == 0
            must_success(
                &einstein_merkle_tree,
                b"k2",
                15,
                0,
                0,
                r,
                false,
                false,
                uncommitted(100, 17, false),
            );
            // caller_start_ts != 0
            must_success(
                &einstein_merkle_tree,
                b"k2",
                15,
                18,
                0,
                r,
                false,
                false,
                uncommitted(100, 17, false),
            );
            // current_ts != 0
            must_success(
                &einstein_merkle_tree,
                b"k2",
                15,
                0,
                18,
                r,
                false,
                false,
                uncommitted(100, 17, false),
            );
            // caller_start_ts != 0 && current_ts != 0
            must_success(
                &einstein_merkle_tree,
                b"k2",
                15,
                19,
                20,
                r,
                false,
                false,
                uncommitted(100, 17, false),
            );
            // caller_start_ts == u64::MAX
            must_success(
                &einstein_merkle_tree,
                b"k2",
                15,
                TimeStamp::max(),
                20,
                r,
                false,
                false,
                uncommitted(100, 17, false),
            );
            // current_ts == u64::MAX
            must_success(
                &einstein_merkle_tree,
                b"k2",
                15,
                20,
                TimeStamp::max(),
                r,
                false,
                false,
                uncommitted(100, 17, false),
            );
            // force_sync_commit = true
            must_success(
                &einstein_merkle_tree,
                b"k2",
                15,
                20,
                TimeStamp::max(),
                r,
                true,
                false,
                |s| s == TtlExpire,
            );
            must_undaggered(&einstein_merkle_tree, b"k2");
            must_get_rollback_protected(&einstein_merkle_tree, b"k2", 15, true);

            // case 3: pessimistic transaction with two keys (large solitontxn), secondary is prewritten first
            must_acquire_pessimistic_dagger_for_large_solitontxn(&einstein_merkle_tree, b"k3", b"k3", 20, 20, 100);
            must_acquire_pessimistic_dagger_for_large_solitontxn(&einstein_merkle_tree, b"k4", b"k3", 20, 25, 100);
            must_pessimistic_prewrite_put_async_commit(
                &einstein_merkle_tree,
                b"k4",
                b"v",
                b"k3",
                &Some(vec![]),
                20,
                25,
                true,
                28,
            );
            // the client must call check_solitontxn_status with caller_start_ts == current_ts == 0, should not push
            must_success(
                &einstein_merkle_tree,
                b"k3",
                20,
                0,
                0,
                r,
                false,
                false,
                uncommitted(100, 21, false),
            );

            // case 4: pessimistic transaction with two keys (not large solitontxn), secondary is prewritten first
            must_acquire_pessimistic_dagger_with_ttl(&einstein_merkle_tree, b"k5", b"k5", 30, 30, 100);
            must_acquire_pessimistic_dagger_with_ttl(&einstein_merkle_tree, b"k6", b"k5", 30, 35, 100);
            must_pessimistic_prewrite_put_async_commit(
                &einstein_merkle_tree,
                b"k6",
                b"v",
                b"k5",
                &Some(vec![]),
                30,
                35,
                true,
                36,
            );
            // the client must call check_solitontxn_status with caller_start_ts == current_ts == 0, should not push
            must_success(
                &einstein_merkle_tree,
                b"k5",
                30,
                0,
                0,
                r,
                false,
                false,
                uncommitted(100, 0, false),
            );
        };

        do_test(true);
        do_test(false);
    }

    fn test_check_solitontxn_status_impl(rollback_if_not_exist: bool) {
        let einstein_merkle_tree = Testeinstein_merkle_treeBuilder::new().build().unwrap();

        let (k, v) = (b"k1", b"v1");

        let r = rollback_if_not_exist;

        let ts = TimeStamp::compose;

        // Try to check a not exist thing.
        if r {
            must_success(
                &einstein_merkle_tree,
                k,
                ts(3, 0),
                ts(3, 1),
                ts(3, 2),
                r,
                false,
                false,
                |s| s == DaggerNotExist,
            );
            // A protected rollback record will be written.
            must_get_rollback_protected(&einstein_merkle_tree, k, ts(3, 0), true);
        } else {
            must_err(&einstein_merkle_tree, k, ts(3, 0), ts(3, 1), ts(3, 2), r, false, false);
        }

        // Dagger the key with TTL=100.
        must_prewrite_put_for_large_solitontxn(&einstein_merkle_tree, k, v, k, ts(5, 0), 100, 0);
        // The initial min_commit_ts is start_ts + 1.
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(5, 0), 100, ts(5, 1), false);

        // CheckTxnStatus with caller_start_ts = 0 and current_ts = 0 should just return the
        // information of the dagger without changing it.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            0,
            0,
            r,
            false,
            false,
            uncommitted(100, ts(5, 1), false),
        );

        // Update min_commit_ts to current_ts.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(6, 0),
            ts(7, 0),
            r,
            false,
            false,
            uncommitted(100, ts(7, 0), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(5, 0), 100, ts(7, 0), false);

        // Update min_commit_ts to caller_start_ts + 1 if current_ts < caller_start_ts.
        // This case should be impossible. But if it happens, we prevents it.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(9, 0),
            ts(8, 0),
            r,
            false,
            false,
            uncommitted(100, ts(9, 1), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(5, 0), 100, ts(9, 1), false);

        // caller_start_ts < dagger.min_commit_ts < current_ts
        // When caller_start_ts < dagger.min_commit_ts, no need to update it, but pushed should be
        // true.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(8, 0),
            ts(10, 0),
            r,
            false,
            false,
            uncommitted(100, ts(9, 1), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(5, 0), 100, ts(9, 1), false);

        // current_ts < dagger.min_commit_ts < caller_start_ts
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(11, 0),
            ts(9, 0),
            r,
            false,
            false,
            uncommitted(100, ts(11, 1), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(5, 0), 100, ts(11, 1), false);

        // For same caller_start_ts and current_ts, update min_commit_ts to caller_start_ts + 1
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(12, 0),
            ts(12, 0),
            r,
            false,
            false,
            uncommitted(100, ts(12, 1), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(5, 0), 100, ts(12, 1), false);

        // Logical time is also considered in the comparing
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(13, 1),
            ts(13, 3),
            r,
            false,
            false,
            uncommitted(100, ts(13, 3), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(5, 0), 100, ts(13, 3), false);

        must_commit(&einstein_merkle_tree, k, ts(5, 0), ts(15, 0));
        must_undaggered(&einstein_merkle_tree, k);

        // Check committed key will get the commit ts.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(12, 0),
            ts(12, 0),
            r,
            false,
            false,
            committed(ts(15, 0)),
        );
        must_undaggered(&einstein_merkle_tree, k);

        must_prewrite_put_for_large_solitontxn(&einstein_merkle_tree, k, v, k, ts(20, 0), 100, 0);

        // Check a committed transaction when there is another dagger. Expect getting the commit ts.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(12, 0),
            ts(12, 0),
            r,
            false,
            false,
            committed(ts(15, 0)),
        );

        // Check a not existing transaction, the result depends on whether `rollback_if_not_exist`
        // is set.
        if r {
            must_success(
                &einstein_merkle_tree,
                k,
                ts(6, 0),
                ts(12, 0),
                ts(12, 0),
                r,
                false,
                false,
                |s| s == DaggerNotExist,
            );
            // And a rollback record will be written.
            must_seek_write(
                &einstein_merkle_tree,
                k,
                ts(6, 0),
                ts(6, 0),
                ts(6, 0),
                WriteType::Rollback,
            );
        } else {
            must_err(&einstein_merkle_tree, k, ts(6, 0), ts(12, 0), ts(12, 0), r, false, false);
        }

        // TTL check is based on physical time (in ms). When logical time's difference is larger
        // than TTL, the dagger won't be resolved.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(20, 0),
            ts(21, 105),
            ts(21, 105),
            r,
            false,
            false,
            uncommitted(100, ts(21, 106), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(20, 0), 100, ts(21, 106), false);

        // If physical time's difference exceeds TTL, dagger will be resolved.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(20, 0),
            ts(121, 0),
            ts(121, 0),
            r,
            false,
            false,
            |s| s == TtlExpire,
        );
        must_undaggered(&einstein_merkle_tree, k);
        must_seek_write(
            &einstein_merkle_tree,
            k,
            TimeStamp::max(),
            ts(20, 0),
            ts(20, 0),
            WriteType::Rollback,
        );

        // Push the min_commit_ts of pessimistic daggers.
        must_acquire_pessimistic_dagger_for_large_solitontxn(&einstein_merkle_tree, k, k, ts(4, 0), ts(130, 0), 200);
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(4, 0), 200, ts(130, 1), true);
        must_success(
            &einstein_merkle_tree,
            k,
            ts(4, 0),
            ts(135, 0),
            ts(135, 0),
            r,
            false,
            false,
            uncommitted(200, ts(135, 1), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(4, 0), 200, ts(135, 1), true);

        // Commit the key.
        must_pessimistic_prewrite_put(&einstein_merkle_tree, k, v, k, ts(4, 0), ts(130, 0), true);
        must_commit(&einstein_merkle_tree, k, ts(4, 0), ts(140, 0));
        must_undaggered(&einstein_merkle_tree, k);
        must_get_commit_ts(&einstein_merkle_tree, k, ts(4, 0), ts(140, 0));

        // Now the transactions are intersecting:
        // T1: start_ts = 5, commit_ts = 15
        // T2: start_ts = 20, rollback
        // T3: start_ts = 4, commit_ts = 140
        must_success(
            &einstein_merkle_tree,
            k,
            ts(4, 0),
            ts(10, 0),
            ts(10, 0),
            r,
            false,
            false,
            committed(ts(140, 0)),
        );
        must_success(
            &einstein_merkle_tree,
            k,
            ts(5, 0),
            ts(10, 0),
            ts(10, 0),
            r,
            false,
            false,
            committed(ts(15, 0)),
        );
        must_success(
            &einstein_merkle_tree,
            k,
            ts(20, 0),
            ts(10, 0),
            ts(10, 0),
            r,
            false,
            false,
            |s| s == RolledBack,
        );

        // Rollback expired pessimistic dagger.
        must_acquire_pessimistic_dagger_for_large_solitontxn(&einstein_merkle_tree, k, k, ts(150, 0), ts(150, 0), 100);
        must_success(
            &einstein_merkle_tree,
            k,
            ts(150, 0),
            ts(160, 0),
            ts(160, 0),
            r,
            false,
            false,
            uncommitted(100, ts(160, 1), true),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(150, 0), 100, ts(160, 1), true);
        must_success(
            &einstein_merkle_tree,
            k,
            ts(150, 0),
            ts(160, 0),
            ts(260, 0),
            r,
            false,
            false,
            |s| s == TtlExpire,
        );
        must_undaggered(&einstein_merkle_tree, k);
        // Rolling back a pessimistic dagger should leave Rollback mark.
        must_seek_write(
            &einstein_merkle_tree,
            k,
            TimeStamp::max(),
            ts(150, 0),
            ts(150, 0),
            WriteType::Rollback,
        );

        // Rollback when current_ts is u64::max_value()
        must_prewrite_put_for_large_solitontxn(&einstein_merkle_tree, k, v, k, ts(270, 0), 100, 0);
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(270, 0), 100, ts(270, 1), false);
        must_success(
            &einstein_merkle_tree,
            k,
            ts(270, 0),
            ts(271, 0),
            TimeStamp::max(),
            r,
            false,
            false,
            |s| s == TtlExpire,
        );
        must_undaggered(&einstein_merkle_tree, k);
        must_seek_write(
            &einstein_merkle_tree,
            k,
            TimeStamp::max(),
            ts(270, 0),
            ts(270, 0),
            WriteType::Rollback,
        );

        must_acquire_pessimistic_dagger_for_large_solitontxn(&einstein_merkle_tree, k, k, ts(280, 0), ts(280, 0), 100);
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(280, 0), 100, ts(280, 1), true);
        must_success(
            &einstein_merkle_tree,
            k,
            ts(280, 0),
            ts(281, 0),
            TimeStamp::max(),
            r,
            false,
            false,
            |s| s == TtlExpire,
        );
        must_undaggered(&einstein_merkle_tree, k);
        must_seek_write(
            &einstein_merkle_tree,
            k,
            TimeStamp::max(),
            ts(280, 0),
            ts(280, 0),
            WriteType::Rollback,
        );

        // Don't push lightlike_completion the min_commit_ts if the min_commit_ts of the dagger is 0.
        must_acquire_pessimistic_dagger_with_ttl(&einstein_merkle_tree, k, k, ts(290, 0), ts(290, 0), 100);
        must_success(
            &einstein_merkle_tree,
            k,
            ts(290, 0),
            ts(300, 0),
            ts(300, 0),
            r,
            false,
            false,
            uncommitted(100, TimeStamp::zero(), false),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(290, 0), 100, TimeStamp::zero(), true);
        pessimistic_rollback::tests::must_success(&einstein_merkle_tree, k, ts(290, 0), ts(290, 0));

        must_prewrite_put_impl(
            &einstein_merkle_tree,
            k,
            v,
            k,
            &None,
            ts(300, 0),
            false,
            100,
            TimeStamp::zero(),
            1,
            /* min_commit_ts */ TimeStamp::zero(),
            /* max_commit_ts */ TimeStamp::zero(),
            false,
            fdbhikvproto::fdbhikvrpcpb::Assertion::None,
            fdbhikvproto::fdbhikvrpcpb::AssertionLevel::Off,
        );
        must_success(
            &einstein_merkle_tree,
            k,
            ts(300, 0),
            ts(310, 0),
            ts(310, 0),
            r,
            false,
            false,
            uncommitted(100, TimeStamp::zero(), false),
        );
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(300, 0), 100, TimeStamp::zero(), false);
        must_rollback(&einstein_merkle_tree, k, ts(300, 0), false);

        must_prewrite_put_for_large_solitontxn(&einstein_merkle_tree, k, v, k, ts(310, 0), 100, 0);
        must_large_solitontxn_daggered(&einstein_merkle_tree, k, ts(310, 0), 100, ts(310, 1), false);
        // Don't push lightlike_completion the min_commit_ts if caller_start_ts is max, but pushed should be true.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(310, 0),
            TimeStamp::max(),
            ts(320, 0),
            r,
            false,
            false,
            uncommitted(100, ts(310, 1), true),
        );
        must_commit(&einstein_merkle_tree, k, ts(310, 0), ts(315, 0));
        must_success(
            &einstein_merkle_tree,
            k,
            ts(310, 0),
            TimeStamp::max(),
            ts(320, 0),
            r,
            false,
            false,
            committed(ts(315, 0)),
        );
    }

    #[test]
    fn test_check_solitontxn_status() {
        test_check_solitontxn_status_impl(false);
        test_check_solitontxn_status_impl(true);
    }

    #[test]
    fn test_check_solitontxn_status_resolving_pessimistic_dagger() {
        let einstein_merkle_tree = Testeinstein_merkle_treeBuilder::new().build().unwrap();
        let k = b"k1";
        let v = b"v1";
        let ts = TimeStamp::compose;

        // Check with resolving_pessimistic_dagger flag.
        // local_path: there is no commit or rollback record, no rollback record should be written.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(3, 0),
            ts(3, 0),
            ts(4, 0),
            true,
            false,
            true,
            |s| s == DaggerNotExistDoNothing,
        );
        must_get_rollback_ts_none(&einstein_merkle_tree, k, ts(5, 0));

        // local_path: there is no commit or rollback record, error should be reported if
        // rollback_if_not_exist is set to false.
        must_err(&einstein_merkle_tree, k, ts(3, 0), ts(5, 0), ts(5, 0), false, false, true);

        // local_path: the pessimistic primary key dagger does exist, and it's not expired yet.
        must_acquire_pessimistic_dagger_with_ttl(&einstein_merkle_tree, k, k, ts(10, 0), ts(10, 0), 10);
        must_pessimistic_daggered(&einstein_merkle_tree, k, ts(10, 0), ts(10, 0));
        must_success(
            &einstein_merkle_tree,
            k,
            ts(10, 0),
            ts(11, 0),
            ts(11, 0),
            true,
            false,
            true,
            uncommitted(10, TimeStamp::zero(), false),
        );

        // local_path: the pessimistic primary key dagger does exist, and it's expired, the primary dagger will
        // be pessimistically rolled back but there will not be a rollback record.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(10, 0),
            ts(21, 0),
            ts(21, 0),
            true,
            false,
            true,
            |s| s == PessimisticRollBack,
        );
        must_undaggered(&einstein_merkle_tree, k);
        must_get_rollback_ts_none(&einstein_merkle_tree, k, ts(22, 0));

        // local_path: the prewrite primary key dagger does exist, and it's not expired yet.
        // Should return daggered status.
        must_prewrite_put_impl(
            &einstein_merkle_tree,
            k,
            v,
            k,
            &None,
            ts(30, 0),
            false,
            10,
            TimeStamp::zero(),
            1,
            /* min_commit_ts */ TimeStamp::zero(),
            /* max_commit_ts */ TimeStamp::zero(),
            false,
            fdbhikvproto::fdbhikvrpcpb::Assertion::None,
            fdbhikvproto::fdbhikvrpcpb::AssertionLevel::Off,
        );
        must_success(
            &einstein_merkle_tree,
            k,
            ts(30, 0),
            ts(31, 0),
            ts(31, 0),
            true,
            false,
            true,
            uncommitted(10, TimeStamp::zero(), false),
        );

        // local_path: the prewrite primary key expired and the solving key is a pessimistic dagger,
        // rollback record should be written and the transaction status is certain.
        must_success(
            &einstein_merkle_tree,
            k,
            ts(30, 0),
            ts(41, 0),
            ts(41, 0),
            true,
            false,
            true,
            |s| s == TtlExpire,
        );
        must_undaggered(&einstein_merkle_tree, k);
        must_get_rollback_ts(&einstein_merkle_tree, k, ts(30, 0));

        // local_path: the resolving_pessimistic_dagger is false and the primary key dagger is pessimistic
        // dagger, the transaction is in commit phase and the rollback record should be written.
        must_acquire_pessimistic_dagger_with_ttl(&einstein_merkle_tree, k, k, ts(50, 0), ts(50, 0), 10);
        must_pessimistic_daggered(&einstein_merkle_tree, k, ts(50, 0), ts(50, 0));
        must_success(
            &einstein_merkle_tree,
            k,
            ts(50, 0),
            ts(61, 0),
            ts(61, 0),
            true,
            false,
            /* resolving_pessimistic_dagger */ false,
            |s| s == TtlExpire,
        );
        must_undaggered(&einstein_merkle_tree, k);
        must_get_rollback_ts(&einstein_merkle_tree, k, ts(50, 0));
    }
}
