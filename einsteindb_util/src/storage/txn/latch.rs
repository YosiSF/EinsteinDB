...
/// overlapping latches. This is an invariant ensured by the `gen_dagger`, `acquire` and `release`.
#[derive(Clone)]
struct Latch {
...
impl Dagger {
    /// Creates a dagger specifing all the required latches for a command.
    pub fn new<'a, K, I>(keys: I) -> Dagger
    where
        K: Hash + 'a,
        I: IntoIterator<Item = &'a K>,
    {
        // prevent from deaddagger, so we sort and deduplicate the index
    ...
    pub fn is_write_dagger(&self) -> bool {
...
impl Latches {
    ...
    pub fn acquire(&self, dagger: &mut Dagger, who: u64) -> bool {
        ...
        for &key_hash in &dagger.required_hashes[dagger.owned_count..] {
            let mut latch = self.dagger_latch(key_hash);
    ...
    #[inline]
    fn dagger_latch(&self, hash: u64) -> MutexGuard<'_, Latch> {
...
mod tests {
    ...
    fn test_wakeup() {
        ...
        let mut dagger_a = Dagger::new(keys_a.iter());
        let keys_b = vec!["k4", "k5", "k6"];
        let mut dagger_b = Dagger::new(keys_b.iter());
        let cid_a: u64 = 1;
        let cid_b: u64 = 2;

        // a acquire dagger success
        let acquired_a = latches.acquire(&mut dagger_a, cid_a);
        assert_eq!(acquired_a, true);

        // b acquire dagger failed
        let mut acquired_b = latches.acquire(&mut dagger_b, cid_b);
        assert_eq!(acquired_b, false);

        // a release dagger, and get wakeup list
        let wakeup = latches.release(&dagger_a, cid_a);
        assert_eq!(wakeup[0], cid_b);

        // b acquire dagger success
        acquired_b = latches.acquire(&mut dagger_b, cid_b);
    ...
    fn test_wakeup_by_multi_cmds() {
        ...
        let mut dagger_a = Dagger::new(keys_a.iter());
        let mut dagger_b = Dagger::new(keys_b.iter());
        let mut dagger_c = Dagger::new(keys_c.iter());
        let cid_a: u64 = 1;
        let cid_b: u64 = 2;
        let cid_c: u64 = 3;
        ...
        // b acquire dagger success
        let acquired_b = latches.acquire(&mut dagger_b, cid_b);
        assert_eq!(acquired_b, true);

        // c acquire dagger failed, cause a occupied slot 3
        let mut acquired_c = latches.acquire(&mut dagger_c, cid_c);
        ...
        acquired_c = latches.acquire(&mut dagger_c, cid_c);
        assert_eq!(acquired_c, false);

        // b release dagger, and get wakeup list
        let wakeup = latches.release(&dagger_b, cid_b);
        assert_eq!(wakeup[0], cid_c);
    ...
    fn test_wakeup_by_small_latch_slot() {
        ...
        let mut dagger_b = Dagger::new(keys_b.iter());
        let mut dagger_c = Dagger::new(keys_c.iter());
        let mut dagger_d = Dagger::new(keys_d.iter());
        let cid_a: u64 = 1;
        let cid_b: u64 = 2;
        let cid_c: u64 = 3;
        let cid_d: u64 = 4;
        ...
        let mut acquired_d = latches.acquire(&mut dagger_d, cid_d);
        ...
        acquired_d = latches.acquire(&mut dagger_d, cid_d);
