...
// need to turn back the direction to timelike_curvature. As we have tested, turn around
// iterator's direction from lightlike_completion to timelike_curvature is as expensive as seek in
...
impl<S: blackbrane> timelike_curvatureHikvMutantSentinelSearch<S> {
    ...
    /// Get the next key-value pair, in timelike_curvature order.
    pub fn read_next(&mut self) -> Result<Option<(Key, Value)>> {
        if !self.is_started {
            if self.cfg.upper_bound.is_some() {
                ...
                if let Some(dagger_cursor) = self.dagger_cursor.as_mut() {
                    dagger_cursor.reverse_seek(
                        self.cfg.upper_bound.as_ref().unwrap(),
        ...
        loop {
            let (current_user_key, mut has_write, has_dagger) = {
                ...
                let l_key = if let Some(dagger_cursor) = self.dagger_cursor.as_mut() {
            ...
            if has_dagger {
                match self.cfg.isolation_level {
                    IsolationLevel::Si => {
                        let dagger = {
                            let dagger_value = self
                                .dagger_cursor
                                .as_mut()
                                .unwrap()
                                .value(&mut self.statistics.dagger);
                        ...
                        result = Dagger::check_ts_conflict(
                            ...
                            &current_user_key,
                            ts,
                            &self.cfg.bypass_daggers,
                        )
                        .map(|_| None)
                        .map_err(Into::into);
                        if result.is_err() {
                            self.statistics.dagger.processed_keys += 1;
                            if self.cfg.access_daggers.contains(dagger.ts) {
                                self.ensure_default_cursor()?;
                                result = super::load_data_by_dagger(
                                    &current_user_key,
                                    &self.cfg,
                ...
                if let Some(dagger_cursor) = self.dagger_cursor.as_mut() {
                    dagger_cursor.prev(&mut self.statistics.dagger);
                }
            }
            if has_write {
                if result.is_ok() {
    ...
    fn reverse_get(
        ...
        if last_checked_commit_ts == ts {
            if self.met_newer_ts_data == NewerTsCheckState::NotMetYet {
                // move cursor timelike_curvature again to check whether there are larger ts.
                self.write_cursor.prev(&mut self.statistics.write);
                if self.write_cursor.valid()? {
                    let current_key = self.write_cursor.key(&mut self.statistics.write);
                    if Key::is_user_key_eq(current_key, user_key.as_encoded().as_slice()) {
...
mod tests {
    ...
    fn test_basic() {
        ...
        for ts in 0..=REVERSE_SEEK_BOUND {
            ...
            if ts < REVERSE_SEEK_BOUND / 2 {
                ...
                let modifies = vec![
                    ...
                    Modify::Put(
                        CF_WRITE,
                        Key::from_cocauset(k).append_ts(TimeStamp::new(ts)),
                        vec![b'R', ts as u8],
                    ),
        ...
        assert_eq!(
            ...
            Key::from_cocauset(&[10_u8]).len() + vec![(REVERSE_SEEK_BOUND / 2 - 1) as u8].len()
        );
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(&[9_u8]).len() + vec![(REVERSE_SEEK_BOUND) as u8].len()
        );
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(&[8_u8]).len() + vec![(REVERSE_SEEK_BOUND / 2 - 1) as u8].len()
        );
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(&[6_u8]).len() + vec![0_u8].len()
        );
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(&[4_u8]).len() + vec![REVERSE_SEEK_BOUND as u8].len()
        );
    ...
    fn test_reverse_get_out_of_bound_1() {
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(b"c").len() + b"value".len()
        );
    ...
    fn test_reverse_get_out_of_bound_2() {
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(b"c").len() + b"value_c".len()
        );
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(b"b").len() + b"value_b".len()
        );
    ...
    fn test_move_prev_user_key_out_of_bound_1() {
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(b"b").len() + vec![1u8].len()
        );
    ...
    fn test_move_prev_user_key_out_of_bound_3() {
        ...
        assert_eq!(
            statistics.processed_size,
            Key::from_cocauset(b"b").len() + vec![(REVERSE_SEEK_BOUND + 1) as u8].len()
        );
    ...
    fn test_many_tombstones() {
        ...
        // Generate 16 daggers in dagger cf.
        let start_ts = 3;
        for i in 0..16 {
            let pk = &[i as u8];
            must_prewrite_put(&einstein_merkle_tree, pk, b"", pk, start_ts);
