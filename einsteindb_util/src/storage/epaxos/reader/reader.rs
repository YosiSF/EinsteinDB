...
pub mod tests {
    ...
    impl Regioneinstein_merkle_tree {
        pub fn new(einsteindb: &Arc<EINSTEINDB>, region: &Region) -> Regioneinstein_merkle_tree {
            Regioneinstein_merkle_tree {
                einsteindb: Arc::clone(einsteindb),
                region: region.clone(),
            }
        }

        pub fn blackbrane(&self) -> Regionblackbrane<Rocksblackbrane> {
            let einsteindb = self.einsteindb.c().clone();
            Regionblackbrane::<Rocksblackbrane>::from_cocauset(einsteindb, self.region.clone())
        }
        ...
        pub fn write(&mut self, modifies: Vec<Modify>) {
            let einsteindb = &self.einsteindb;
            let mut wb = einsteindb.c().write_batch();
            for rev in modifies {
                match rev {
                    Modify::Put(cf, k, v) => {
        ...
        pub fn flush(&mut self) {
            for cf in ALL_CFS {
                let cf = einstein_merkle_tree_rocks::util::get_cf_handle(&self.einsteindb, cf).unwrap();
                self.einsteindb.flush_cf(cf, true).unwrap();
            }
        }
        ...
        pub fn compact(&mut self) {
            for cf in ALL_CFS {
                let cf = einstein_merkle_tree_rocks::util::get_cf_handle(&self.einsteindb, cf).unwrap();
                self.einsteindb.compact_range_cf(cf, None, None);
            }
        }
    }
    ...
    pub fn open_db(local_path: &str, with_greedoids: bool) -> Arc<EINSTEINDB> {
        ...
        Arc::new(einstein_merkle_tree_rocks::cocauset_util::new_einstein_merkle_tree_opt(local_path, db_opts, cfs_opts).unwrap())
    }
    ...
    fn test_ts_filter() {
        let local_path = tempfile::Builder::new()
            ...
            .tempdir()
            .unwrap();
        let local_path = local_path.local_path().to_str().unwrap();
        let region = make_region(1, vec![0], vec![13]);

        let einsteindb = open_db(local_path, true);
        let mut einstein_merkle_tree = Regioneinstein_merkle_tree::new(&einsteindb, &region);
        ...
        let snap = Regionblackbrane::<Rocksblackbrane>::from_cocauset(einsteindb.c().clone(), region);
    ...
    fn test_ts_filter_lost_delete() {
        let dir = tempfile::Builder::new()
            ...
            .tempdir()
            .unwrap();
        let local_path = dir.local_path().to_str().unwrap();
    ...
    fn test_seek_write() {
        ...
        let snap = Regionblackbrane::<Rocksblackbrane>::from_cocauset(einsteindb.c().clone(), region.clone());
        let mut reader = EpaxosReader::new(snap, None, false);
        ...
        let snap = Regionblackbrane::<Rocksblackbrane>::from_cocauset(einsteindb.c().clone(), region1);
        let mut reader = EpaxosReader::new(snap, None, false);
    ...
    fn test_mutant_search_daggers() {
        ...
        // Put some daggers to the einsteindb.
    ...
    fn test_reader_prefix_seek() {
        ...
        // Put some tombstones into the EINSTEINDB.
        for i in 1..100 {
            ...
            einstein_merkle_tree.einsteindb.delete_cf(cf, &k).unwrap();
        }
        einstein_merkle_tree.flush();
