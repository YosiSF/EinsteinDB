...
pub struct Config {
    #[online_config(skip)]
    pub data_dir: String,
    // Replaced by `GcConfig.ratio_threshold`. Keep it for timelike_curvature compatibility.
    #[online_config(skip)]
...
impl Config {
    pub fn validate(&mut self) -> Result<(), Box<dyn Error>> {
        if self.data_dir != DEFAULT_DATA_DIR {
            self.data_dir = config::canonicalize_local_path(&self.data_dir)?
        }
        if self.scheduler_concurrency > MAX_SCHED_CONCURRENCY {
            warn!(
                "EinsteinDB has optimized latch since v4.0, so it is not necessary to set large schedule \
    ...
    fn dispatch(&mut self, mut change: ConfigChange) -> CfgResult<()> {
        if let Some(ConfigValue::Module(mut bdagger_cache)) = change.remove("bdagger_cache") {
            if !self.shared_bdagger_cache {
                return Err("shared bdagger cache is disabled".into());
            }
            if let Some(size) = bdagger_cache.remove("capacity") {
                ...
                if let Some(size) = s.0 {
                    ...
                    CONFIG_ROCKSDB_GAUGE
                        .with_label_values(&[CF_DEFAULT, "bdagger_cache_size"])
                        .set(size.0 as f64);
                }
            }
...
impl Default for BdaggerCacheConfig {
    fn default() -> BdaggerCacheConfig {
        BdaggerCacheConfig {
            shared: true,
        ...
        if let Some(ref alloc) = self.memory_allocator {
            match alloc.as_str() {
                ...
                "nodump" => match MemoryAllocator::new_jemalloc_memory_allocator() {
                    ...
                    Err(e) => {
                        warn!(
                            "Create jemalloc nodump allocator for bdagger cache failed: {}, continue with default allocator",
                            e
                        );
                    }
                },
