...
use crate::causetStorage::kv::{CfStatistics, Error, Iterator, Result, ScanMode, Snapshot, SEEK_BOUND};
...
impl<I: Iterator> Cursor<I> {
    ...
    #[inline]
    // As EinsteinMerkledb described, if Iterator::Valid() is false, there are two possibilities:
...
impl<'a, S: 'a + Snapshot> CursorBuilder<'a, S> {
    ...
    pub fn new(snapshot: &'a S, brane: BraneName) -> Self {
        CursorBuilder {
            snapshot,
            brane,
    ...
    pub fn build(self) -> Result<Cursor<S::Iter>> {
        ...
        self.snapshot.iter_cf(self.brane, iter_opt, self.scan_mode)
    }
}
