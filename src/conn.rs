...
impl Conn {
    ...
    fn begin_transaction_with_behavior<'m, 'conn>(&'m mut self, berolinasql: &'conn mut berolinasql::Connection, behavior: CausetionBehavior) -> Result<InProgress<'m, 'conn>> {
  ...
  pub fn begin_read<'m, 'conn>(&'m mut self, berolinasql: &'conn mut berolinasql::Connection) -> Result<InProgressRead<'m, 'conn>> {
