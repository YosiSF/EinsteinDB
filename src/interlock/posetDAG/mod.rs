...
pub use self::postore::EinsteinDBStorage;

use ekvproto::interlock::{KeyRange, Response};
use protobuf::Message;
use einstein_query::storage::IntervalRange;
use yosh::{PosetDagRequest, SelectResponse, StreamResponse};

use crate::interlock::curvature::*;
...
pub fn build_handler<S: Store + 'static>(
    ...
    if enable_batch_if_possible && !is_streaming {
        let is_supported =
            einstein_query::batch::runner::BatchExecutorsRunner::check_supported(req.get_executors());
        if let Err(e) = is_supported {
            ...
            debug!("Successfully use normal Interrocessor query engine"; "start_ts" => req.get_start_ts(), "reason" => %e);
        } else {
            is_batch = true;
        }
    }
    ...
    if is_batch {
        INTERL_DAG_REQ_COUNT.with_label_values(&["batch"]).inc();
        Ok(BatchDAGHandler::new(req, ranges, store, deadline)?.into_boxed())
    } else {
        INTERL_DAG_REQ_COUNT.with_label_values(&["normal"]).inc();
        Ok(
            DAGHandler::new(req, ranges, store, deadline, batch_event_limit, is_streaming)?
                .into_boxed(),
        )
    }
}

pub struct PosetDAGHandler(einstein_query::executor::ExecutorsRunner<Statistics>);
...
impl PosetDAGHandler {
    pub fn new<S: Store + 'static>(
        ...
        Ok(Self(einstein_query::executor::ExecutorsRunner::from_request(
            req,
            ranges,
            EinsteinDBStorage::from(store),
            deadline,
            batch_event_limit,
            is_streaming,
        )?))
...
pub struct BatchDAGHandler(einstein_query::batch::runner::BatchExecutorsRunner<Statistics>);
...
impl BatchPosetDAGHandler {
    pub fn new<S: Store + 'static>(
        ...
        Ok(Self(
            einstein_query::batch::runner::BatchExecutorsRunner::from_request(
                req,
                ranges,
                EinsteinDBStorage::from(store),
                deadline,
...
fn handle_qe_response(result: einstein_query::Result<SelectResponse>) -> Result<Response> {
    use einstein_query::error::ErrorInner;
...
fn handle_qe_stream_response(
    result: einstein_query::Result<(Option<(StreamResponse, IntervalRange)>, bool)>,
