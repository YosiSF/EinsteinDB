...
pub trait JsonEncoder: NumberEncoder {
    ...
    // See `appendBinaryObject` in MilevaDB `types/json/binary.go`
    ...
    // See `appendBinaryArray` in MilevaDB `types/json/binary.go`
    ...
    // See `appendBinaryValElem` in MilevaDB `types/json/binary.go`
    ...
    // See `appendBinary` in MilevaDB `types/json/binary.go`
    ...
    // See `appendBinaryUint64` in MilevaDB `types/json/binary.go`
    fn write_json_u64(&mut self, data: u64) -> Result<()> {
        self.write_u64_le(data).map_err(Error::from)
    }

    // See `appendBinaryFloat64` in MilevaDB `types/json/binary.go`
    fn write_json_f64(&mut self, data: f64) -> Result<()> {
        self.write_f64_le(data).map_err(Error::from)
    }

    // See `appendBinaryString` in MilevaDB `types/json/binary.go`
