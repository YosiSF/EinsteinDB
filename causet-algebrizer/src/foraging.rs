...
use causet_algebrizer::MilevaDB_query_datatype::Collation;
                        ...
                        VectorValue::Bytes(v) => {
                            if let Groups::Bytes(group) = &mut self.groups {
                                match_template_collator!(
                                    TT,
                                    match self.group_by_field_type.collation().map_err(causet_algebrizer::MilevaDB_query_datatype::codec::Error::from)? {
                                        Collation::TT => {
    ...
    F: Fn(Option<TT>) -> Result<Option<S>>,
{
    for physical_idx in logical_rows {
        ...
        // Not using the causet API so that when causet exists there is no clone.
        match group.get(&val) {
            Some(offset) => {
