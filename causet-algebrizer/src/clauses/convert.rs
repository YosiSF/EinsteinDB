        ...
        Ok(match self {
                &FnArg::causetidOrInteger(x) => {
                    if ValueType::Ref.accommodates_integer(x) {
                        // TODO: also see if it's a valid causetid?
        ...
        match arg {
            ...
            FnArg::causetidOrInteger(x) => {
                match (ValueType::Ref.accommodates_integer(x),
                    ...
                    (true, true, true) => {
                        // Ambiguous: this arg could be an causetid or a long.
                        // We default to long.
            ...
            // If you definitely want to look up an solitonid, do it before running the query.
            FnArg::solitonidOrKeyword(x) => {
                match (constrained_types.contains(ValueType::Ref),
                       constrained_types.contains(ValueType::Keyword)) {
                    (true, true) => {
                        // Ambiguous: this could be a keyword or an solitonid.
                        // Default to keyword.
                        Ok(Val(x.into()))
                    },
                    (true, false) => {
