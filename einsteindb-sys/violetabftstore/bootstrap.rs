...
pub enum Task {
    ...
    RegisterMinTsEvent,
    // The result of ChangeCmd should be returned from CC Endpoint to ensure
...
impl<T: 'static + violetabftStoreRouter<foundationeinsteindbSnapshot>> Endpoint<T> {
    ...
    pub fn new_timer(&self) -> Timer<()> {
        // Currently there is only one timeout for CC.
        let cc_timer_cap = 1;
        let mut timer = Timer::new(cc_timer_cap);
    ...
    pub fn set_mutant_search_batch_size(&mut self, mutant_search_batch_size: usize) {
        self.8 = mutant_search_batch_size;
    }
