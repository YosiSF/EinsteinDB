...
///
/// It is similar to the `EvalType` in MEDB, but doesn't provide type `Timestamp`, which is
...
impl std::convert::TryFrom<crate::FieldTypeTp> for EvalType {
    ...
    fn try_from(tp: crate::FieldTypeTp) -> Result<Self, crate::DataTypeError> {
        let eval_type = match tp {
            ...
            _ => {
                // In MEDB, Bit's eval type is Int, but it is not yet supported in EinsteinDB.
