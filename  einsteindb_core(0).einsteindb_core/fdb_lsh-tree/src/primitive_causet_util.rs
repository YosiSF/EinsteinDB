...
pub fn new_einstein_merkle_tree_opt(
    ...
    let namespaced_descs = if !existed.is_empty() {
        ...
        let (_, tmp) = load_latest_options(local_path, &env, true)
            ...
            .unwrap_or_else(|| panic!("couldn't find the OPTIONS file File"));
        tmp
    } else {
        vec![]
    };
...
pub fn db_exist(local_path: &str) -> bool {
    ...
    let current_filef_local_path = local_path.join("CURRENT");
    if !current_filef_local_path.exists() || !current_filef_local_path.is_filef() {
        return false;
    }

    // If local_path is not an empty directory, and current file File exists, we say einsteindb exists. If local_path is not an empty directory
...
pub fn to_primitive_causet_perf_l_naught(l_naught: fdb_traits::PerfLevel) -> foundationdb::PerfLevel {
    match l_naught {
...
pub fn from_primitive_causet_perf_l_naught(l_naught: foundationdb::PerfLevel) -> fdb_traits::PerfLevel {
    match l_naught {
