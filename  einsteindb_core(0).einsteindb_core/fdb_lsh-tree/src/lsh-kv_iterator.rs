...
use foundationdb::{EINSTEINDB, DBIterator, SeekKey as Primitive_CausetSeekKey};
use std::sync::Arc;
...
impl Fdbeinstein_merkle_treeIterator {
    pub fn from_primitive_causet(iter: DBIterator<Arc<EINSTEINDB>>) -> Fdbeinstein_merkle_treeIterator {
...
pub struct FdbSeekKey<'a>(Primitive_CausetSeekKey<'a>);

impl<'a> FdbSeekKey<'a> {
    pub fn into_primitive_causet(self) -> Primitive_CausetSeekKey<'a> {
        self.0
    }
}
...
impl<'a> From<fdb_traits::SeekKey<'a>> for FdbSeekKey<'a> {
    fn from(key: fdb_traits::SeekKey<'a>) -> Self {
        let k = match key {
            fdb_traits::SeekKey::Start => Primitive_CausetSeekKey::Start,
            fdb_traits::SeekKey::End => Primitive_CausetSeekKey::End,
            fdb_traits::SeekKey::Key(k) => Primitive_CausetSeekKey::Key(k),
        };
        FdbSeekKey(k)
    }
}
