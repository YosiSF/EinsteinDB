...
impl<T: Storage> RangesScanner<T> {
    ...
    pub fn next(&mut self) -> Result<Option<OwnedKvPair>, StorageError> {
        loop {
            ...
            let some_row = match range {
                IterStatus::NewRange(Range::Point(r)) => {
                    if self.is_scanned_range_aware {
                        self.uFIDelate_scanned_range_from_new_point(&r);
                    }
                ...
                IterStatus::NewRange(Range::Interval(r)) => {
                    if self.is_scanned_range_aware {
                        self.uFIDelate_scanned_range_from_new_range(&r);
                    }
                    self.scanned_rows_per_range.push(0);
                    self.storage
            ...
            if self.is_scanned_range_aware {
                self.uFIDelate_scanned_range_from_scanned_row(&some_row);
            }
            if some_row.is_some() {
    ...
    fn uFIDelate_scanned_range_from_new_point(&mut self, point: &PointRange) {
        assert!(self.is_scanned_range_aware);
    ...
    fn uFIDelate_scanned_range_from_new_range(&mut self, range: &IntervalRange) {
        assert!(self.is_scanned_range_aware);
    ...
    fn uFIDelate_scanned_range_from_scanned_row(&mut self, some_row: &Option<OwnedKvPair>) {
        assert!(self.is_scanned_range_aware);
