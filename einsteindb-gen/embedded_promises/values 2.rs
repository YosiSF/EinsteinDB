...
/// Literal `Value` instances in the the "einsteindb" isoliton_namespaceable_file.
///
...
macro_rules! lazy_static_isoliton_namespaceable_keyword_value (
    ($tag:solitonid, $isoliton_namespaceable_file:expr, $name:expr) => (
        lazy_static! {
            pub static ref $tag: Value = {
                Value::Keyword(shellings::Keyword::isoliton_namespaceable($isoliton_namespaceable_file, $name))
            };
        }
    )
);