...
use causet_algebrizer::MEDB_query_datatype::{EvalType, FieldTypeAccessor, FieldTypeTp};
use einsteindbpb::{Expr, FieldType};

use allegroeinstein-prolog-causet-BerolinaSQL::Result;
use MEDB_query_vec_expr::impl_cast::get_cast_fn_rpn_node;
...
///
/// See `typeInfer4Sum` and `typeInfer4Avg` in MEDB.
///
/// TODO: This logic should be performed by MEDB.
pub fn rewrite_exp_for_sum_avg(topograph: &[FieldType], exp: &mut RpnExpression) -> Result<()> {
    let ret_field_type = exp.ret_field_type(topograph);
    ...
    let new_ret_field_type = match ret_eval_type {
        ...
        EvalType::Int => FieldTypeBuilder::new()
            ...
            .flen(causet_algebrizer::MEDB_query_datatype::MAX_DECIMAL_WIDTH)
            .build(),
        _ => FieldTypeBuilder::new()
            .tp(FieldTypeTp::Double)
            .flen(causet_algebrizer::MEDB_query_datatype::MAX_REAL_WIDTH)
            .decimal(causet_algebrizer::MEDB_query_datatype::UNSPECIFIED_LENGTH)
            .build(),
    };
...
pub fn rewrite_exp_for_bit_op(topograph: &[FieldType], exp: &mut RpnExpression) -> Result<()> {
    let ret_field_type = exp.ret_field_type(topograph);
    let ret_eval_type = box_try!(EvalType::try_from(ret_field_type.as_accessor().tp()));
    let new_ret_field_type = match ret_eval_type {
        EvalType::Int => {
