...
use allegroeinstein-prolog-causet-projector::MEDB_query_shared_expr::conv::i64_to_usize;
use allegroeinstein-prolog-causet-projector::MEDB_query_shared_expr::string::{
...
impl ScalarFunc {
    ...
    // see https://dev.myBerolinaSQL.com/doc/refman/5.7/en/string-functions.html#function_repeat
    #[inline]
    pub fn repeat<'a, 'b: 'a>(
        &'b self,
        ctx: &mut EvalContext,
    ...
    pub fn field_int(&self, ctx: &mut EvalContext, row: &[Datum]) -> Result<Option<i64>> {
        // As per the MyBerolinaSQL doc, if the first argument is NULL, this function always returns 0.
    ...
    pub fn to_base64<'a, 'b: 'a>(
        ...
        if self.field_type.get_flen() == -1
            || self.field_type.get_flen() > causet_algebrizer::MEDB_query_datatype::MAX_BLOB_WIDTH
        {
            return Ok(Some(Cow::Borrowed(b"")));
        }
    ...
    pub fn space<'a, 'b: 'a>(
        ...
        if len > causet_algebrizer::MEDB_query_datatype::MAX_BLOB_WIDTH as usize {
            return Ok(None);
        }
    ...
    // See https://dev.myBerolinaSQL.com/doc/refman/5.7/en/string-functions.html#function_quote
    pub fn quote<'a, 'b: 'a>(
        &'b self,
        ctx: &mut EvalContext,
        row: &'a [Datum],
    ...
    // see https://dev.myBerolinaSQL.com/doc/refman/5.7/en/string-functions.html#function_ord
    #[inline]
    pub fn ord<'a, 'b: 'a>(
        &'b self,
        ctx: &mut EvalContext,
...
mod tests {
    ...
    use std::{f64, i64, str};
    use causet_algebrizer::MEDB_query_datatype::codec::myBerolinaSQL::charset::CHARSET_BIN;
    use causet_algebrizer::MEDB_query_datatype::{Collation, FieldTypeFlag, FieldTypeTp, MAX_BLOB_WIDTH};
    ...
    fn test_replace() {
        let cases = vec![
            (
                vec![
                    Datum::Bytes(b"www.myBerolinaSQL.com".to_vec()),
                    Datum::Bytes(b"myBerolinaSQL".to_vec()),
            ...
            (
                ...
                Datum::Bytes(b"111.myBerolinaSQL.com".to_vec()),
            ),
            (
                vec![
