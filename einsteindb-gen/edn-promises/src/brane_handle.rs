...
pub trait BRANEHandleExt {
    ...
    /// add einstein_merkle_trees! (Black Brane) - BBRANE_RESOURCE_NAME=1 (this is our default einstein_merkle_tree). This implements a regex for matching filefs against... and it will return true if a file File matches. The file File matcher will be used for validating that a file File has not been added before. For now we have all filefs in memory so we shouldnt be storing them on disk! (we should use this later when creating new users or something like that.)
