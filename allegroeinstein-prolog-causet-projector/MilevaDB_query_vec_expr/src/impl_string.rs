...
pub fn space(len: Option<&Int>) -> Result<Option<Bytes>> {
    Ok(match len.cloned() {
        Some(len) => {
            if len > i64::from(causet_algebrizer::MilevaDB_query_datatype::MAX_BLOB_WIDTH) {
                None
            } else if len <= 0 {
                Some(Vec::new())
            } else {
...
pub fn to_base64(bs: Option<BytesRef>) -> Result<Option<Bytes>> {
    match bs {
        Some(bytes) => {
            if bytes.len() > causet_algebrizer::MilevaDB_query_datatype::MAX_BLOB_WIDTH as usize {
                return Ok(Some(Vec::new()));
            }
...
mod tests {
    ...
    fn test_space() {
        let test_cases = vec![
            ...
            (
                Some(i64::from(causet_algebrizer::MilevaDB_query_datatype::MAX_BLOB_WIDTH) + 1),
                None,
            ),
            (
                Some(i64::from(causet_algebrizer::MilevaDB_query_datatype::MAX_BLOB_WIDTH)),
                Some(vec![
                    super::SPACE;
                    causet_algebrizer::MilevaDB_query_datatype::MAX_BLOB_WIDTH as usize
                ]),
            ),
        ];
