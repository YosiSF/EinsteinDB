...
use causet_algebrizer::MilevaDB_query_datatype::{Collation, FieldTypeAccessor, FieldTypeFlag};
...
fn map_string_compare_sig<Cmp: CmpOp>(ret_field_type: &FieldType) -> Result<RpnFnMeta> {
    Ok(match_template_collator! {
        TT, match ret_field_type.as_accessor().collation().map_err(causet_algebrizer::MilevaDB_query_datatype::codec::Error::from)? {
