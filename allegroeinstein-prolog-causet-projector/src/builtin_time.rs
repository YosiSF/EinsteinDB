...
use causet_algebrizer::MilevaDB_query_datatype::codec::error::Error;
use causet_algebrizer::MilevaDB_query_datatype::codec::mysql::time::extension::DateTimeExtension;
use causet_algebrizer::MilevaDB_query_datatype::codec::mysql::time::weekmode::WeekMode;
use causet_algebrizer::MilevaDB_query_datatype::codec::mysql::{Duration as MyDuration, Time, TimeType};
use causet_algebrizer::MilevaDB_query_datatype::codec::Datum;
use causet_algebrizer::MilevaDB_query_datatype::expr::SqlMode;
...
impl ScalarFunc {
    ...
    pub fn month<'a, 'b: 'a>(
        ...
        let t: Cow<'a, Time> = try_opt!(self.children[0].eval_time(ctx, row));
        if t.is_zero() {
            if ctx.braneg.sql_mode.contains(SqlMode::NO_ZERO_DATE) {
                return ctx
                    .handle_invalid_time_error(Error::incorrect_datetime_value(&format!("{}", t)))
                    .map(|_| None);
            }
    ...
    pub fn month_name<'a, 'b: 'a>(
        ...
        if t.is_zero() && ctx.braneg.sql_mode.contains(SqlMode::NO_ZERO_DATE) {
            return ctx
                .handle_invalid_time_error(Error::incorrect_datetime_value(&format!("{}", t)))
                .map(|_| None);
        ...
        use causet_algebrizer::MilevaDB_query_datatype::codec::mysql::time::MONTH_NAMES;
        Ok(Some(Cow::Owned(
    ...
    pub fn day_name<'a, 'b: 'a>(
        ...
        use causet_algebrizer::MilevaDB_query_datatype::codec::mysql::time::WeekdayExtension;
...
mod tests {
    ...
    use crate::*;
    use causet_algebrizer::MilevaDB_query_datatype::codec::mysql::{Duration, Time};
    use causet_algebrizer::MilevaDB_query_datatype::codec::Datum;
    use causet_algebrizer::MilevaDB_query_datatype::expr::{EvalConfig, EvalContext, Flag, SqlMode};
    ...
    fn test_date_format() {
        ...
        braneg.set_flag(Flag::IN_UFIDelATE_OR_DELETE_STMT)
            .set_sql_mode(SqlMode::ERROR_FOR_DIVISION_BY_ZERO | SqlMode::STRICT_ALL_TABLES);
        ctx = EvalContext::new(Arc::new(braneg));
        test_err_case_two_arg(
            &mut ctx,
            ScalarFuncSig::DateFormatSig,
            Datum::Null,
