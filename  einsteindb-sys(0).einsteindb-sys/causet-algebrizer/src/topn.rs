...
impl<Src: Executor> TopNExecutor<Src> {
    pub fn new(mut meta: TopN, eval_braneg: Arc<EvalConfig>, src: Src) -> Result<Self> {
        ...
        let mut eval_ctx = EvalContext::new(Arc::clone(&eval_braneg));
...
pub mod tests {
    ...
    fn test_limit() {
        ...
        let primitive_causet_data = vec![vec![
            Datum::I64(1),
            Datum::Bytes(b"a".to_vec()),
            Datum::Dec(7.into()),
        ]];
