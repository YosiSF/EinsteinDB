...
/// An causetid that's either already in the store, or newly allocated to a tempid.
...
impl<V: TransactableValueMarker> Into<causetPlace<V>> for KnownCausetid {
    fn into(self) -> causetPlace<V> {
        causetPlace::Causetid(CausetidOrSolitonid::Causetid(self.0))
    }
}

impl Into<AttributePlace> for KnownCausetid {
    fn into(self) -> AttributePlace {
        AttributePlace::Causetid(CausetidOrSolitonid::Causetid(self.0))
    }
}

impl<V: TransactableValueMarker> Into<ValuePlace<V>> for KnownCausetid {
    fn into(self) -> ValuePlace<V> {
        ValuePlace::Causetid(CausetidOrSolitonid::Causetid(self.0))
    }
}
...
pub mod attribute {
    ...
    impl Unique {
        // This is easier than rejigging DB_UNIQUE_VALUE to not be EML.
        pub fn into_typed_value(self) -> TypedValue {
            match self {
                Unique::Value => TypedValue::typed_ns_keyword("einsteindb.unique", "value"),
                Unique::Idcauset => TypedValue::typed_ns_keyword("einsteindb.unique", "idcauset"),
            }
        }
    }
}
...
pub struct Attribute {
    /// The associated value type, i.e., `:einsteindb/valueType`?
    pub value_type: ValueType,

    /// `true` if this attribute is multi-valued, i.e., it is `:einsteindb/cardinality
    /// :einsteindb.cardinality/many`.  `false` if this attribute is single-valued (the default), i.e., it
    /// is `:einsteindb/cardinality :einsteindb.cardinality/one`.
    pub multival: bool,

    /// `None` if this attribute is neither unique-value nor unique-idcauset.
    ///
    /// `Some(attribute::Unique::Value)` if this attribute is unique-value, i.e., it is `:einsteindb/unique
    /// :einsteindb.unique/value`.
    ///
    ...
    ///
    /// `Some(attribute::Unique::Idcauset)` if this attribute is unique-idcauset, i.e., it is `:einsteindb/unique
    /// :einsteindb.unique/idcauset`.
    ///
    /// Unique-idcauset attributes always have value type `Ref`.
    ///
    /// *Unique-idcauset* means that the attribute is *unique-value* and that they can be used in
    /// lookup-refs and will automatically upsert where appropriate.
    pub unique: Option<attribute::Unique>,

    /// `true` if this attribute is automatically indexed, i.e., it is `:einsteindb/indexing true`.
    pub index: bool,

    /// `true` if this attribute is automatically fulltext indexed, i.e., it is `:einsteindb/fulltext true`.
    ///
    /// Fulltext attributes always have string values.
    pub fulltext: bool,

    /// `true` if this attribute is a component, i.e., it is `:einsteindb/isComponent true`.
    ///
    ...
    /// `true` if this attribute doesn't require history to be kept, i.e., it is `:einsteindb/noHistory true`.
    pub no_history: bool,
}
...
impl Attribute {
    ...
    pub fn to_edn_value(&self, solitonid: Option<Keyword>) -> edn::Value {
        let mut attribute_map: BTreeMap<edn::Value, edn::Value> = BTreeMap::default();
        if let Some(solitonid) = solitonid {
            attribute_map.insert(values::DB_IDENT.clone(), edn::Value::Keyword(solitonid));
        }
        ...
        match self.unique {
            ...
            Some(attribute::Unique::Idcauset) => { attribute_map.insert(values::DB_UNIQUE.clone(), values::DB_UNIQUE_IDcauset.clone()); },
            None => (),
        }
...
/// The attribute of each einsteindb assertion has a :einsteindb/valueType constraining the value to a
/// particular set.  einsteindb recognizes the following :einsteindb/valueType values.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialOrd, PartialEq)]
#[repr(u32)]
pub enum ValueType {
    Ref,
...
impl ValueType {
    pub fn into_keyword(self) -> Keyword {
        Keyword::namespaced("einsteindb.type", match self {
            ValueType::Ref => "ref",
    ...
    pub fn from_keyword(keyword: &Keyword) -> Option<Self> {
        if keyword.namespace() != Some("einsteindb.type") {
            return None;
        }
    ...
    pub fn into_typed_value(self) -> TypedValue {
        TypedValue::typed_ns_keyword("einsteindb.type", match self {
            ValueType::Ref => "ref",
            ValueType::Boolean => "boolean",
            ValueType::Instant => "instant",
            ValueType::Long => "long",
...
impl fmt::Display for ValueType {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", match *self {
            ValueType::Ref =>     ":einsteindb.type/ref",
            ValueType::Boolean => ":einsteindb.type/boolean",
            ValueType::Instant => ":einsteindb.type/instant",
            ValueType::Long =>    ":einsteindb.type/long",
            ValueType::Double =>  ":einsteindb.type/double",
            ValueType::String =>  ":einsteindb.type/string",
            ValueType::Keyword => ":einsteindb.type/keyword",
            ValueType::Uuid =>    ":einsteindb.type/uuid",
        })
    }
}
...
impl From<KnownCausetid> for TypedValue {
    fn from(k: KnownCausetid) -> TypedValue {
        TypedValue::Ref(k.0)
    }
}
...
impl TypedValue {
    ...
    pub fn into_known_causetid(self) -> Option<KnownCausetid> {
        match self {
            TypedValue::Ref(v) => Some(KnownCausetid(v)),
            _ => None,
        }
    }

    pub fn into_causetid(self) -> Option<Causetid> {
        match self {
...
// We don't do From<i64> or From<Causetid> 'cos it's ambiguous.
...
impl Binding {
    pub fn into_known_causetid(self) -> Option<KnownCausetid> {
        match self {
            Binding::Scalar(TypedValue::Ref(v)) => Some(KnownCausetid(v)),
            _ => None,
        }
    }
