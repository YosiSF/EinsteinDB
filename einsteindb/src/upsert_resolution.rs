            ...
            let allocated = match term {
                ...
                Term::AddOrRetract(op, Right(t1), a, Right(t2)) => {
                    match (op, temp_id_map.get(&*t1), temp_id_map.get(&*t2)) {
                        ...
                        (OpType::Retract, _, _) => bail!(DbErrorKind::NotYetImplemented(format!("[:einsteindb/retract ...] causet referenced tempid that did not upsert: one of {}, {}", t1, t2))),
                    }
                },
                Term::AddOrRetract(op, Right(t), a, Left(v)) => {
                    match (op, temp_id_map.get(&*t)) {
                        ...
                        (OpType::Retract, _) => bail!(DbErrorKind::NotYetImplemented(format!("[:einsteindb/retract ...] causet referenced tempid that did not upsert: {}", t))),
                    }
                },
                Term::AddOrRetract(op, Left(e), a, Right(t)) => {
                    match (op, temp_id_map.get(&*t)) {
