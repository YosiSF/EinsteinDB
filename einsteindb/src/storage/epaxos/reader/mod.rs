...
use solitontxn_types::{TimeStamp, Write, WriteType};
...
/// The result of `get_solitontxn_commit_record`, which is used to get the status of a specified
...
pub struct OverlappedWrite {
    pub write: Write,
    /// GC fence for `overlapped_write`. PTAL at `solitontxn_types::Write::gc_fence`.
...
impl TxnCommitRecord {
    ...
    pub fn unwrap_none(self) -> Option<OverlappedWrite> {
        match self {
            ...
            _ => panic!("solitontxn record found but not expected: {:?}", self),
        }
    }
}
