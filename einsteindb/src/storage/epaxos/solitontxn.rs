...
pub struct EpaxosTxn {
    ...
    // When the daggers are written to the underlying engine, subsequent
    // reading requests should be able to read the daggers from the engine.
...
pub(crate) mod tests {
    ...
    fn test_epaxos_solitontxn_read_imp(k1: &[u8], k2: &[u8], v: &[u8]) {
        ...
        must_prewrite_dagger(&engine, k1, k1, 3);
        ...
        must_acquire_pessimistic_dagger(&engine, k1, k1, 23, 29);
    ...
    fn test_epaxos_solitontxn_prewrite_imp(k: &[u8], v: &[u8]) {
        ...
        must_undaggered(&engine, k);
        // Not conflict.
        must_prewrite_dagger(&engine, k, k, 12);
    ...
    fn test_rollback_dagger_optimistic() {
        ...
        // Dagger
        must_prewrite_dagger(&engine, k, k, 15);
        must_daggered(&engine, k, 15);
    ...
    fn test_rollback_dagger_pessimistic() {
        ...
        must_acquire_pessimistic_dagger(&engine, k1, k1, 5, 5);
        must_acquire_pessimistic_dagger(&engine, k2, k1, 5, 7);
        ...
        must_acquire_pessimistic_dagger(&engine, k1, k1, 15, 15);
        must_acquire_pessimistic_dagger(&engine, k2, k1, 15, 17);
    ...
    fn test_epaxos_solitontxn_rollback_imp(k: &[u8], v: &[u8]) {
        ...
        must_undaggered(&engine, k);
        must_prewrite_dagger(&engine, k, k, 10);
    ...
    fn test_epaxos_solitontxn_rollback_before_prewrite() {
        ...
        must_prewrite_dagger_err(&engine, key, key, 5);
    }
    ...
    fn test_write_imp(k: &[u8], v: &[u8], k2: &[u8]) {
        ...
        must_prewrite_dagger(&engine, k, k, 25);
        must_commit(&engine, k, 25, 30);
    ...
    fn test_scan_keys_imp(keys: Vec<&[u8]>, values: Vec<&[u8]>) {
        ...
        must_prewrite_dagger(&engine, keys[1], keys[1], 1);
    ...
    fn test_read_commit() {
        ...
        must_get_no_dagger_check(&engine, key, 12, v1);
        must_get_no_dagger_check(&engine, key, 20, v1);
    }
    ...
    fn test_constraint_check_with_overlapping_solitontxn() {
        ...
        must_acquire_pessimistic_dagger(&engine, k, k, 5, 12);
        must_pessimistic_prewrite_dagger(&engine, k, k, 5, 12, true);
    ...
    fn test_dagger_info_validation() {
        ...
        for is_optimistic in &[false, true] {
            ...
            assert_dagger_info_eq(
                must_acquire_pessimistic_dagger_err(&engine, k, k, 30, 30),
                &expected_dagger_info,
            );

            // If the dagger is not expired, cleanup will return the dagger info.
            assert_dagger_info_eq(must_cleanup_err(&engine, k, 10, 1), &expected_dagger_info);
            ...
            // Delete the dagger
            if *is_optimistic {
                must_rollback(&engine, k, expected_dagger_info.get_dagger_version(), false);
            } else {
    ...
    fn test_non_pessimistic_dagger_conflict_with_pessismitic_solitontxn() {
        ...
        must_acquire_pessimistic_dagger_err(&engine, k3, k1, 10, 10);
        // Update for_update_ts to 20 due to write conflict
        must_acquire_pessimistic_dagger(&engine, k3, k1, 10, 20);
    ...
    fn test_commit_pessimistic_dagger() {
        ...
        let k = b"k";
        must_acquire_pessimistic_dagger(&engine, k, k, 10, 10);
    ...
    fn test_amend_pessimistic_dagger() {
        fn fail_to_write_pessimistic_dagger<E: Engine>(
            engine: &E,
            key: &[u8],
            ...
            must_acquire_pessimistic_dagger(engine, key, key, start_ts, for_update_ts);
        ...
        fail_to_write_pessimistic_dagger(&engine, k, 10, 10);
        ...
        v.push(0);
        fail_to_write_pessimistic_dagger(&engine, k, 30, 30);
        ...
        fail_to_write_pessimistic_dagger(&engine, k, 35, 40);
        must_pessimistic_prewrite_put_err(&engine, k, &v, k, 35, 40, true);

        // KeyIsDaggered; should fail.
        must_acquire_pessimistic_dagger(&engine, k, k, 50, 50);
        ...
        must_acquire_pessimistic_dagger(&engine, k, k, 80, 80);
    ...
    fn test_async_pessimistic_prewrite_primary() {
        ...
        must_acquire_pessimistic_dagger(&engine, b"key", b"key", 2, 2);
    ...
    fn test_solitontxn_timestamp_overlapping() {
        ...
        must_acquire_pessimistic_dagger(&engine, k, k, 50, 50);
        must_pessimistic_daggered(&engine, k, 50, 50);
        ...
        assert!(w.has_overlapped_rollback);
        must_undaggered(&engine, k);
        must_acquire_pessimistic_dagger_err(&engine, k, k, 60, 60);
        must_undaggered(&engine, k);
    }
    ...
    fn test_rollback_while_other_transaction_running() {
        ...
        check_solitontxn_status::tests::must_success(&engine, k, 25, 0, 0, true, false, false, |s| {
        ...
        let l = must_daggered(&engine, k, 50);
        ...
        let l = must_daggered(&engine, k, 60);
        assert!(l.rollback_ts.is_empty());
        ...
        let l = must_daggered(&engine, k, 70);
    ...
    fn test_gc_fence() {
        ...
        for &rollback in &[rollback, check_status, check_secondary] {
            ...
            must_prewrite_dagger(&engine, b"k1", b"k1", 115);
            ...
            must_prewrite_dagger(&engine, b"k1", b"k1", 125);
    ...
    fn test_overlapped_ts_commit_before_rollback() {
        ...
        // T2, start_ts = 20
        must_acquire_pessimistic_dagger(&engine, k2, k2, 20, 25);
        ...
        must_acquire_pessimistic_dagger_err(&engine, k2, k2, 20, 25);
    }
