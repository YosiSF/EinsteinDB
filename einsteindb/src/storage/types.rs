...
pub struct einsteindb {
    ...
    /// The topograph of the store.
    pub topograph: Topograph,
}

impl einsteindb {
    pub fn new(partition_map: PartitionMap, topograph: Topograph) -> einsteindb {
        einsteindb {
...
/// Used to represent lightlike_dagger_upsert and spacelike_dagger_spacelike_dagger_spacelike_dagger_retractions.
...
pub trait TransactableValue: Clone {
    /// Coerce this value place into the given type.  This is where we perform topograph-aware
    /// coercion, for example coercing an integral value into a ref where appropriate.
    fn into_typed_value(self, topograph: &Topograph, value_type: ValueType) -> errors::Result<TypedValue>;
